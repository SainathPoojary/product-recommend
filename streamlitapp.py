...
def main():
    ...
    # Read clean dataset
    retail = pd.read_csv("./data/CleanRetailData.csv")
    ...
    def kmeans_on_df():
        ...
        rfm_scaled = pd.DataFrame(
            rfm_scaled, columns=["Recency", "Frequency", "Monetary", "RFM_Score"]
        )

        # Fit Kmeans at n_clusters = 4
        kmeans = KMeans(
            n_clusters=4, init="k-means++", n_init=10, max_iter=50, verbose=0
        )
    ...
    def plot_pcts(df, string):
        ...
        fig_target = go.Figure(
            data=[go.Pie(labels=df.index, values=df[string], hole=0.3)]
        )
        fig_target.update_layout(
            showlegend=False, height=500, margin={"l": 10, "r": 10, "t": 0, "b": 0}
        )
        fig_target.update_traces(textposition="inside", textinfo="label+percent")
        fig_target.update_traces(
            marker=dict(colors=["lightcyan", "cyan", "royalblue", "darkblue"])
        )
    ...
    def group_sales_quantity(df, feature):
        df = (
            df[[f"{feature}", "Quantity", "Sales Revenue"]]
            .groupby([f"{feature}"])
            .sum()
    ...
    if page == "Product Recommendation using Market Basket Analysis":
        ...
        # Opening JSON file
        with open("./data/item_sets.json") as json_file:
            data = json.load(json_file)

        # Display
        if len(data[prod_option]) == 0:
            st.error(
                "Oops! No product recommendations available yet! Please select a different item."
            )
        else:
            st.markdown("####")
    ...
    if page == "Customer Segmentation based on RFM Analysis":
        ...
        try:
            ...
            with col1:
                ...
                st.markdown(
                    "It is a customer segmentation technique that uses past purchase behavior to segment customers. To perform RFM analysis, we divide customers into four equal groups according to the distribution of values for recency, frequency, and monetary value."
                )
            ...
            with col3:
                ...
                rfm_country_df["InvoiceDate"] = pd.to_datetime(
                    rfm_country_df["InvoiceDate"]
                )
                ref_date = rfm_country_df["InvoiceDate"].max() + dt.timedelta(days=1)

                # Remove 'Guest Customer'
                rfm_country_df = rfm_country_df[
                    rfm_country_df["CustomerID"] != "Guest Customer"
                ]

                # Aggregating over CustomerID
                rfm_new = rfm_country_df.groupby("CustomerID").agg(
                    {
                ...
                rfm_new["R"] = pd.qcut(
                    rfm_new["Recency"].rank(method="first"), 4, labels=[4, 3, 2, 1]
                )
                rfm_new["F"] = pd.qcut(
                    rfm_new["Frequency"].rank(method="first"), 4, labels=[1, 2, 3, 4]
                )
                rfm_new["M"] = pd.qcut(
                    rfm_new["Monetary"].rank(method="first"), 4, labels=[1, 2, 3, 4]
                )

                # Calculate RFM Score
                rfm_new["RFM_Score"] = (
                    rfm_new["R"].astype(int)
            ...
            # Pie charts
            rfm_stats = (
                df[["Clusters", "RFM_Score", "Recency", "Frequency", "Monetary"]]
                .groupby("Clusters")
                .agg(["mean"])
            )
            rfm_stats.columns = [
    ...
    if page == "Dashboard":
        ...
        top_customers = (
            country_retail[country_retail["CustomerID"] != "Guest Customer"]
        ...
        with col1:
            ...
            st.markdown("###")
            top_products_qty = (
                group_sales_quantity(country_retail, "Description")
        ...
        with col3:
            ...
            top_products_revenue = (
                group_sales_quantity(country_retail, "Description")
                .sort_values(ascending=False, by="Sales Revenue")
                .reset_index(drop=True)
            )
        ...
        with col1:
            ...
            # Plotting
            fig = px.bar(
                country_data_wo_uk_subset, x="Country", y="InvoiceNo", color="InvoiceNo"
            )
            fig.update_layout(
                showlegend=False,
                height=400,
        ...
        #     # Plotting
        #     fig = px.bar(
        #         sales_wo_uk, x="Sales Revenue", y="Country", color="Sales Revenue"
        #     )
        #     fig.update_layout(
        #         showlegend=False,
        #         height=400,
        ...
        with col1:
            ...
            # Plotting
            fig = px.bar(
                monthly_stats, x="Sales Revenue", y="Month", color="Sales Revenue"
            )
            fig.update_layout(
                showlegend=False,
                height=400,
        ...
        with col1:
            ...
            fig_target = go.Figure(
                data=[
                    go.Pie(
                        labels=daily_stats["Day of Week"],
        ...
        with col1:
            ...
            fig_target = go.Figure(
                data=[
                    go.Pie(
                        labels=timeofday_stats["Time of Day"],
